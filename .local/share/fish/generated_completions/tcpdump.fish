...
complete -c tcpdump -s k --description 'Control the display of packet metadata via an optional  metadata_arg argument.'
...
complete -c tcpdump -s Q --description 'To specify a filter expression based on packet metadata information like inte…'
...
complete -c tcpdump -l time-zone-offset --description 'To set the time zone where the packet capture file was saved.'
