...
complete -c pcre2grep -s N -l newline --description 'Six different conventions for indicating the ends of lines in scanned files a…'
...
complete -c pcre2grep -l om-capture --description 'Set the number of capturing parentheses that can be accessed by -o.'
...
complete -c pcre2grep -s u -l utf --description 'Operate in UTF-8 mode.'
complete -c pcre2grep -s U -l utf-allow-invalid --description 'As --utf, but in addition subject lines may contain invalid UTF-8 code unit s…'
...
complete -c pcre2grep -o o0 --description 'given without an argument (see above), if an argument is present, it must be …'
